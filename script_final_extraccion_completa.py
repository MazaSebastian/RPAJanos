...
# -*- coding: utf-8 -*-
"""
Script final para extracción completa de datos del evento
Incluye todos los datos requeridos: Homenajeada/o, Código, Tipo evento, Fecha, Salón, Cliente, Celular, Celular 2, Tipo Pack
"""
...
def extraer_datos_evento_completo():
    """Extraer todos los datos del evento de manera completa y precisa"""
    
    print("=== EXTRACCIÓN COMPLETA DE DATOS DEL EVENTO ===")
    print("Objetivo: Extraer todos los datos requeridos del evento 33069")
    ...
    try:
        ...
        # PASO 10: EXTRAER TODOS LOS DATOS REQUERIDOS
        print(f"\n=== PASO 10: EXTRAER TODOS LOS DATOS REQUERIDOS ===")
        ...
        # 2. CÓDIGO DE EVENTO
        print("--- BUSCANDO CÓDIGO DE EVENTO ---")
        for i, elem in enumerate(elementos_relevantes):
            if "Codigo de Evento" in elem['texto']:
                if i + 1 < len(elementos_relevantes):
                    siguiente_elem = elementos_relevantes[i + 1]
                    if siguiente_elem['texto'] and siguiente_elem['tag'] == 'td':
                        datos_evento['codigo_evento'] = siguiente_elem['texto']
                        print(f"✓ Código de evento: {datos_evento['codigo_evento']}")
                        break
        ...
        # 7. CELULAR Y CELULAR 2 (Buscar números específicos)
        print("--- BUSCANDO CELULAR Y CELULAR 2 ---")
        
        # Buscar números que empiecen con 5411 (formato argentino)
        patron_celular = r'5411\d{8}'
        celulares_encontrados = []
        
        for elem in elementos_relevantes:
            texto = elem['texto']
            matches = re.findall(patron_celular, texto)
            for match in matches:
                if match not in celulares_encontrados:
                    celulares_encontrados.append(match)
                    print(f"✓ Celular encontrado: {match}")
        ...
        elif len(celulares_encontrados) == 1:
            # Si solo hay un celular, usar el mismo para ambos
            datos_evento['celular_2'] = celulares_encontrados[0]
        ...
        for campo, valor in datos_evento.items():
            if valor and campo not in ['fecha', 'url']:
                datos_encontrados += 1
                print(f"✓ {campo}: {valor}")
            elif not valor and campo not in ['fecha', 'url']:
...
if __name__ == "__main__":
    resultado = extraer_datos_evento_completo()
    if resultado:
        print(f"\n=== RESULTADO FINAL ===")
